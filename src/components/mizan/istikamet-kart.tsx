...
export function IstikametKart({ ad, yuzde, metin, ikon: Ikon, renkVar, to, rozet, onClick }: Props) {
  ...
  const ortakSinif =
    "group relative flex w-full flex-col items-center gap-5 overflow-hidden rounded-2xl border border-border bg-card px-5 py-7 text-left transition-all hover:border-[color:var(--border)]";
  ...
  return (
    <div className="relative">
      ...
      {onClick ? (
        <button type="button" onClick={onClick} className={ortakSinif} style={ortakStil}>
          {icerik}
        </button>
      ) : (
        <Link to={to} className={ortakSinif} style={ortakStil}>
          {icerik}
        </Link>
      )}
    </div>
